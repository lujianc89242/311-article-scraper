...
for each_url in url_list[1301:1302]:
    url = each_url
    article_id = url[45:]
    print(url[45:])
    ...
    if(len(cards)==1):
        ...
        card_body = ka_content.find("div",{"class":"card-body"})
        # parsing the 1st card
        # The 1st card always has no card-head and section1 could have a divTable for alert
        section_tags = []
        for section in card_body.contents:
            if(str(type(section)) == "<class 'bs4.element.Tag'>"): #anything besides bs4.element.Tag will be NavigableString, which just like regular strings
                section_tags.append(section)
